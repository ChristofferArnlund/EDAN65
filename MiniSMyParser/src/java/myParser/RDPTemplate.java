 ...
 * convenience methods to integrate a scanner, like peeking the next token or
 * reading a certain token. Subclasses must implement the parseProgram() method,
 * which is used as entry point for recursive decent parsing.
 */
public abstract class RDPTemplate {
	 ...
	 * 
	 * @param scanner
	 *            providing the token stream to process; used throughout parsing
	 * @return 
	 * @throws RDCException 
	 * @throws RuntimeException
	 *             if the program is not syntactically correct
	 */
	public Object parse(LangScanner scanner) {
	...
	/**
	 * Read the next token (invokes the scanner to set the current token to the
	 * next).
	 */
	public void accept() {
		try {
	...
	public void accept(int expectedToken) {
		if (!peek(expectedToken)) {
			error("expected token " + LangParser.Terminals.NAMES[expectedToken] + " got token "
