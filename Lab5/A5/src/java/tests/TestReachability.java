...
public class TestReachability extends AbstractParameterizedTest {
	/**
	 * Directory where test files live
	 */
	private static final String TEST_DIR = "testfiles/reachability";

	/**
	 * Construct a new JastAdd test
	 * @param filename filename of test input file
	 */
	public TestReachability(String filename) {
		...
		try{
			...
			if(errors.size() == 0) { //no error detected in analysis continue
				for(Func func : program.getFuncs()) {
					sb.append("reach("+func.getFuncName().getID() + ") = {");
					Iterator<Func> reachable = func.reachable().iterator();
					while(reachable.hasNext()) {
						sb.append(reachable.next().getFuncName().getID());
						if(reachable.hasNext())
							sb.append(", ");
					}
					sb.append("}\n");
				} 
